...
# img = cv2.imread('tire.tif');
# m = np.zeros(img.shape);
# m[33:33+117,44:44+128] = 1;
# seg = region_seg(img,m,500);

def chanvese3d_yield(I,init_mask,max_its=200,alpha=0.2,thresh=0):
    I = I.astype('float')

    # -- Create a signed distance map (SDF) from mask
    phi = mask2phi(init_mask)
    phi=phi.astype('float')
    # phi = init_mask.astype('float')
    yield phi
    # --main loop
    its = 0
    stop = False
    ...
    while (its < max_its and not stop):
        ...
        if len(idx) > 0:
            ...
            if np.mod(its, 10) == 0:
                print('iteration:',
                      its)  # set(ud.txtInfo1,'string',sprintf('iteration: %d',its),'color',[1 1 0]);  # drawnow;
...
def sussman(D, dt):
    ...
    # print(a.shape,D.shape,type(a))
    i_max = D.shape[0]*D.shape[1]*D.shape[2]
    a_p=np.where(a_p<0,0,a_p)
    a_n=np.where(a_n>0,0,a_n)
    b_p = np.where(b_p < 0, 0, b_p)
    b_n = np.where(b_n > 0, 0, b_n)
    c_p = np.where(c_p < 0, 0, c_p)
    c_n = np.where(c_n > 0, 0, c_n)
    d_p = np.where(d_p < 0, 0, d_p)
    d_n = np.where(d_n > 0, 0, d_n)
    e_p = np.where(e_p < 0, 0, e_p)
    e_n = np.where(e_n > 0, 0, e_n)
    f_p = np.where(f_p < 0, 0, f_p)
    f_n = np.where(f_n > 0, 0, f_n)
...
if __name__=="__main__":
    ...
    mask[x//2-50:x//2+50,y//2-50:y//2+50,z//2-50:z//2+50]=1


    chanvese3d(img, mask, max_its=2000, display=True, alpha=1)